...
const IronManSuit = (props: any) => {
  ...
  const { armorRed, armorGold, armorSilver, reactorBlue } = useMemo(() => {
    const red = new MeshPhysicalMaterial({
      ...
      roughness: 0.3, // Slightly more roughness for realistic falloff
      clearcoat: 1.0,
    ...
    const blue = new MeshStandardMaterial({
      color: "#00FFFF",
      emissive: "#00FFFF",
      emissiveIntensity: 10, // Increased for Bloom
      toneMapped: false
    });
  ...
  return (
    <group ref={group} {...props} dispose={null}>
      <group ref={torsoRef}>
        {/* Torso */}
        <mesh position={[0, 0.2, 0]} material={armorRed} castShadow receiveShadow>
          <cylinderGeometry args={[0.35, 0.25, 0.5, 32]} />
        </mesh>
        <mesh position={[0, 0.35, 0.15]} rotation={[0.2, 0, 0]} material={armorGold} castShadow receiveShadow>
          <RoundedBox args={[0.55, 0.2, 0.15]} radius={0.05} smoothness={4} />
        </mesh>
        <mesh position={[-0.2, 0.4, 0.1]} rotation={[0.4, 0, 0.2]} material={armorSilver}>
          <RoundedBox args={[0.1, 0.05, 0.2]} radius={0.02} smoothness={4} />
        </mesh>
        <mesh position={[0.2, 0.4, 0.1]} rotation={[0.4, 0, -0.2]} material={armorSilver}>
          <RoundedBox args={[0.1, 0.05, 0.2]} radius={0.02} smoothness={4} />
        </mesh>
        ...
        {/* Abdomen */}
        <mesh position={[0, -0.2, 0]} material={armorGold} castShadow receiveShadow>
          <cylinderGeometry args={[0.24, 0.22, 0.4, 32]} />
        </mesh>
        <mesh position={[0, -0.2, 0.12]} material={armorRed}>
          <RoundedBox args={[0.2, 0.3, 0.05]} radius={0.02} smoothness={4} />
        </mesh>
        ...
        <group ref={headRef} position={[0, 0.55, 0]}>
          ...
          <mesh position={[0, 0.1, 0]} material={armorRed} castShadow receiveShadow>
            <RoundedBox args={[0.28, 0.35, 0.32]} radius={0.08} smoothness={8} />
          </mesh>
          <mesh position={[0, 0.1, 0.165]} material={armorGold} castShadow receiveShadow>
        ...
        {/* Shoulders */}
        <mesh position={[-0.45, 0.35, 0]} material={armorGold} castShadow receiveShadow><sphereGeometry args={[0.18, 32, 32]} /></mesh>
        <mesh position={[0.45, 0.35, 0]} material={armorGold} castShadow receiveShadow><sphereGeometry args={[0.18, 32, 32]} /></mesh>

        {/* Right Arm */}
        <group ref={rightArmRef} position={[0.45, 0.35, 0]}>
          {/* UPPER ARM: GOLD */}
          <mesh position={[0.1, -0.3, 0]} rotation={[0, 0, -0.2]} material={armorGold} castShadow receiveShadow>
            <cylinderGeometry args={[0.1, 0.09, 0.45, 32]} />
          </mesh>
          {/* Right Forearm Group (Attached to elbow pivot area) */}
          <group ref={rightForearmRef} position={[0.15, -0.6, 0]}>
            <mesh position={[0, -0.2, 0]} material={armorGold} castShadow receiveShadow><RoundedBox args={[0.14, 0.4, 0.14]} radius={0.03} smoothness={4} /></mesh>
            <mesh position={[0, -0.45, 0]} material={armorRed} castShadow receiveShadow><RoundedBox args={[0.1, 0.15, 0.05]} radius={0.02} smoothness={4} /></mesh>
            <mesh position={[0, -0.45, 0]} rotation={[Math.PI / 2, 0, 0]} material={reactorBlue} castShadow receiveShadow><circleGeometry args={[0.03]} /></mesh>
      ...
      <group position={[0, -0.4, 0]}>
        <mesh position={[0, 0, 0]} material={armorRed} castShadow receiveShadow><RoundedBox args={[0.4, 0.25, 0.25]} radius={0.05} smoothness={4} /></mesh>
        <group position={[-0.2, -0.5, 0]}>
          <mesh material={armorGold}><cylinderGeometry args={[0.13, 0.1, 0.7, 32]} /></mesh>
          <group position={[0, -1.3, 0.15]} rotation={[0.4, 0, 0]}>
            <mesh material={armorRed}><RoundedBox args={[0.15, 0.1, 0.25]} radius={0.02} smoothness={4} /></mesh>
